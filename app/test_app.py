...
def test_create_item_without_name(client):
    ...
    item_data = {"description": "Missing name"}
...
def test_update_nonexistent_item(client):
    ...
    response = client.put(
        "/api/items/9999", data=json.dumps(update_data), content_type="application/json"
    )
...
def test_full_crud_workflow(client):
    ...
    create_response = client.post(
        "/api/items", data=json.dumps(create_data), content_type="application/json"
    )
